  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((particle, i) => {
        ...
        // Attract particles to center with subtle force
        ...
        if (distance > maxDistance) {
          const angle = Math.atan2(dy, dx);
          particle.x = centerX + Math.cos(angle) * (radius * 0.5);
          particle.y = centerY + Math.sin(angle) * (radius * 0.5);
        }
        ...
        particles.forEach((other, j) => {
          ...
          if (distance < 120) {
            ...
            ctx.strokeStyle = `rgba(241, 89, 42, ${opacity})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        });
