        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                href={item.path}
                className={`relative px-2 py-1 transition-colors ${pathname === item.path ? 'text-[#f1592a]' : 'text-gray-700 hover:text-[#f1592a]'}`}
                key={item.path}
              >
                {item.name}
                {pathname === item.path && (
          ...
          <nav className="container mx-auto px-4 py-4 flex flex-col space-y-4">
            {navItems.map((item) => (
              <Link
                ...
                className={`px-4 py-2 rounded-lg transition-colors ${pathname === item.path ? 'bg-[#f1592a] text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                key={item.path}
              >
                {item.name}
              </Link>
