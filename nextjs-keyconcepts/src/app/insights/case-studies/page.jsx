      ...
      {staticCaseStudies.map((caseStudy) => (
        <div className="group" key={caseStudy.id}>
          <Link href={caseStudy.link} className="block h-full">
            <div className="h-full bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col hover:-translate-y-1">
              <div className="relative h-60 overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <span className="inline-block px-3 py-1 bg-[#f1592a] text-white text-xs font-bold rounded-full shadow-md">
                    {caseStudy.client}
                  </span>
                </div>
              ...
              <div className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-3 group-hover:text-[#f1592a] transition-colors leading-snug">
                    {caseStudy.title}
                  </h3>
                ...
                <div>
                  {caseStudy.metrics && caseStudy.metrics.length > 0 && (
                    <div className="grid grid-cols-3 gap-2 py-4 mb-6 border-y border-gray-100 bg-gray-50/50 rounded-xl px-2">
                      {caseStudy.metrics.map((metric, idx) => (
                        <div className="text-center" key={idx}>
                          <div className="text-base sm:text-lg font-bold text-[#f1592a]">
                            {metric.value}
                          </div>
                          <div className="text-[11px] text-gray-500 font-medium line-clamp-2 leading-tight">
                            {metric.label}
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="flex items-center justify-between text-[#f1592a] font-semibold text-sm">
...
function DynamicCaseStudies({ featuredCase, regularCases }) {
  return (
    <>
      {featuredCase && (
      ...
      <section className="py-12 md:py-16 bg-white">
        <div className="w-full max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <CaseStudiesList caseStudies={regularCases} />
        </div>
      </section>
    </>
  )
...
async function CaseStudiesPage() {
  const featuredCase = USE_STATIC_CASE_STUDIES ? null : await getFeaturedCaseStudy()
  const regularCases = USE_STATIC_CASE_STUDIES ? [] : await getRegularCaseStudies()
      ...
      <div className="min-h-screen bg-white">
        <section className="relative pt-32 pb-20 overflow-hidden bg-gradient-to-br from-gray-50 to-white">
          ...
          <div className="relative w-full max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {USE_STATIC_CASE_STUDIES ? (
              <StaticCaseStudies />
            ) : (
              <DynamicCaseStudies featuredCase={featuredCase} regularCases={regularCases} />
            )}
          </div>
        </section>
